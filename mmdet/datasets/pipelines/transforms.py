...
class RandomFlip(object):
    ...
    def bbox_rotate90(self, bboxes, img_shape):

        assert bboxes.shape[-1] % 4 == 0
        w = img_shape[1]
        h = img_shape[0]
        rotated = bboxes.copy()
        rotated[...,0::4] = h - bboxes[...,3::4]
        rotated[...,1::4] = bboxes[...,0::4]
        rotated[...,2::4] = h - bboxes[...,1::4]
        rotated[...,3::4] = bboxes[...,2::4]
        return rotated
    def bbox_rotate180(self, bboxes, img_shape):

        assert bboxes.shape[-1] % 4 == 0
        w = img_shape[1]
        h = img_shape[0]
        rotated = bboxes.copy()
        rotated[...,0::4] = w - bboxes[...,2::4]
        rotated[...,1::4] = h - bboxes[...,3::4]
        rotated[...,2::4] = w - bboxes[...,0::4]
        rotated[...,3::4] = h - bboxes[...,1::4]
        return rotated
    def bbox_rotate270(self, bboxes, img_shape):

        assert bboxes.shape[-1] % 4 == 0
        w = img_shape[1]
        h = img_shape[0]
        rotated = bboxes.copy()
        rotated[...,0::4] = bboxes[...,1::4]
        rotated[...,1::4] = w - bboxes[...,2::4]
        rotated[...,2::4] = bboxes[...,3::4]
        rotated[...,3::4] = w - bboxes[...,0::4]
        return rotated
    ...
    def __call__(self, results):
        ...
        if ROTATE4:
            ...
            if  rotate_possibility <= 0.25:
                rotate = 0
            elif rotate_possibility>0.25 and rotate_possibility<=0.5:
                rotate = 1
            elif rotate_possibility>0.5 and rotate_possibility<=0.75:
                rotate = 2
            else:
                rotate = 3
            if rotate==1:
                # rotate image
                results['img'] = mmcv.imrotate(results['img'],90)
                # rotate bboxes
                for key in results.get('bbox_fields', []):
                    results[key] = self.bbox_rotate90(results[key],
                                                results['img_shape'])
                '''
                # how to rotate masks
                for key in results.get('mask_fields', []):
                ...
                # rotate bboxes
                for key in results.get('bbox_fields', []):
                    results[key] = self.bbox_rotate180(results[key],
                                                results['img_shape'])
            elif rotate==3:
                # rotate image
                results['img'] = mmcv.imrotate(results['img'],270)
                # rotate bboxes
                for key in results.get('bbox_fields', []):
